...

{
  ...
  he: {
    hero: {
      badge: 'REST API + MCP · WhatsApp Business API',
      h1: 'WhatsApp Analytics API — אנליטיקס',
      subhead:
        'ההבדל בין AI שיכול <strong style="color:#e9edef">לשלוח</strong> וואטסאפ לבין אחד שיכול <strong style="color:#e9edef">להבין</strong> אותו. תשאלו על המסרים, השיחות, הקמפיינים וה־CRM שלכם — בנוי כדי להיקרא על ידי סוכני AI.',
      primary: { label: 'צור חשבון חינם →' },
      secondary: { label: 'קרא את תיעוד ה־API' },
      chips: ['הודעות', 'שיחות', 'לידים ופניות', 'תוצאות קמפיינים'],
    },
    blocks: [
      {
        title: 'שאלות שאפשר באמת לענות עליהן',
        intro: 'כל אחת ממופה לנקודת קצה / מדד אנליטיקס אמיתי של גמבוט — שום דבר לא מומצא.',
        bullets: [
          '"כמה הודעות קיבלנו / שלחנו היום?" — <code>GET /analytics/summary</code>, <code>/analytics/messages</code>.',
          '"כמה שיחות ממתינות למענה?" — <code>GET /conversations/sla</code>.',
          '"כמה לידים חדשים / מה מצב הפייפליין?" — <code>GET /analytics/leads</code>.',
          '"מה קרה בשירות הלקוחות היום?" — <code>GET /analytics/cases</code> (+ SLA).',
          '"מה היו השאלות הנפוצות של הלקוחות?" — <code>GET /analytics/transcript</code> (קראו את התוכן בפועל ואז סכמו).',
          '"איך הצליח הקמפיין?" — <code>GET /campaigns/{id}/results</code>.',
        ],
      },
      {
        title: 'מבנה אנליטיקס קריא ל־AI',
        intro: 'התגובות מובנות כך שסוכן לא צריך לשחזר דבר. כתובת בסיס: ' + API_BASE + '.',
      },
      {
        title: 'טווחי זמן ואזור זמן ברורים',
        paragraphs: [
          'סוכנים מבקשים "היום", "אתמול", "השבוע" או "30 הימים האחרונים". נקודות הקצה של האנליטיקס מקבלות קיצור <code>period</code> או תאריכי <code>from</code>/<code>to</code> מפורשים ותמיד מחזירות את הטווח שנפתר, כך שהסוכן יודע בדיוק מה מדד. גבולות היום עוקבים אחר אזור הזמן העסקי של החשבון — בלי לנחש איזה "היום" הכוונה.',
          'נתונים רגישים לעולם אינם נחשפים אוטומטית: האנליטיקס מכבד את הרשאות מפתח ה־API, ותוכן הודעות זמין רק דרך נקודת קצה התמלול המפורשת (שדורשת את ההרשאה המתאימה).',
        ],
      },
      {
        title: 'עם סוכן AI (MCP)',
        prompts: [
          { user: 'כמה הודעות וואטסאפ קיבלנו היום?', agent: '147 התקבלו; 32 לידים חדשים; 11 ממתינות למענה. (להמחשה — ערכים אמיתיים מגיעים מנקודות הקצה של האנליטיקס שלכם.)' },
          { user: 'אילו לקוחות עדיין ממתינים למענה?' },
          { user: 'סכם את שיחות שירות הלקוחות של היום.' },
        ],
        note: 'המספרים לדוגמה הם להמחשה בלבד, לא נתוני גמבוט.',
      },
      {
        title: 'תנו ל־AI שלכם הבנה אמיתית של וואטסאפ',
        text: 'צרו חשבון חינם, חברו וואטסאפ, ותנו לסוכנים לתשאל את התפעול שלכם — לא רק לשלוח הודעות.',
        primary: { label: 'צור חשבון חינם →' },
        secondary: { label: 'WhatsApp API לסוכני AI' },
      },
    ],
    faq: [
      { q: 'אילו מדדי וואטסאפ אפשר לתשאל דרך API?', a: 'נפח הודעות (התקבלו/נשלחו), שיחות הממתינות למענה (SLA), אנשי קשר חדשים, פייפליין לידים, פניות/כרטיסים, משימות, ביצועי מודעות CTWA, הרצות בוט ותוצאות קמפיינים — כל אחד דרך נקודת קצה אנליטיקס ייעודית.' },
      { q: 'איך מטופלים טווחי זמן?', a: 'השתמשו בקיצור period (today, yesterday, week, month, 30d, all) או בתאריכי from/to מפורשים; התגובה מחזירה את הטווח שנפתר. גבולות היום עוקבים אחר אזור הזמן העסקי של החשבון.' },
      { q: 'האם AI יכול לקרוא את תוכן השיחה בפועל?', a: 'כן, דרך נקודת קצה התמלול (עם ההרשאה המתאימה) — שימושי לשאלות איכותניות כמו "על מה לקוחות שאלו היום?". גישה לתוכן מכבדת את הרשאות מפתח ה־API.' },
      { q: 'האם המספרים לדוגמה הם נתוני גמבוט אמיתיים?', a: 'לא — המספרים בדף זה הם להמחשה. ערכים אמיתיים מחושבים מנתוני החשבון שלכם.' },
    ],
  },
};

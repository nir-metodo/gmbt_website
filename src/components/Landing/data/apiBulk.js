...

{
  ...
  he: {
    hero: {
      badge: 'REST API + MCP · WhatsApp Business API',
      h1: 'WhatsApp Bulk Messaging API — דיוור המוני',
      subhead:
        'שלחו וואטסאפ <strong style="color:#e9edef">בקנה מידה, בדרך הנכונה</strong>. השתמשו בקמפיינים (לעולם לא בלולאת שליחה), התאימו תבניות לכל נמען, כבדו את מגבלות המסרים של Meta, והחריגו אוטומטית אנשי קשר שביקשו הסרה.',
      primary: { label: 'צור חשבון חינם →' },
      secondary: { label: 'קרא את תיעוד ה־API' },
      chips: ['קמפיינים', 'מגיליון נתונים', 'מגבלות מסרים', 'בטוח להסרה'],
    },
    blocks: [
      {
        title: 'לא ללולאה — השתמשו בקמפיין',
        paragraphs: [
          'שליחת הודעות רבות בלולאה על נקודת שליחה בודדת היא גישה שגויה: היא מתעלמת ממגבלות תפוקה, הסכמה, התאמה אישית לכל נמען ודיווח. ה־API וה־MCP של גמבוט מפורשים בעניין — כשמזוהה קהל גדול, אתם מונחים להשתמש ב<strong style="color:#e9edef">קמפיין</strong> במקום.',
          'קמפיינים בונים את הקהל (תגיות, רשימות, פלחי CRM או גיליון), מתאימים את התבנית המאושרת לכל נמען, ומחזירים תוצאות מסירה לכל נמען — הכל בתוך כללי Meta.',
        ],
      },
      {
        title: 'שליחה המונית מרשימה',
        intro: 'כתובת בסיס: ' + API_BASE + '. השתמשו ב־dryRun כדי לתצוגה מקדימה של הקהל והספירות תחילה.',
      },
      {
        title: 'מנגנוני הגנה מובנים',
        bullets: [
          '<strong style="color:#e9edef">מגבלות מסרים של Meta</strong> — התגובות חושפות את המדרגה היומית שלכם; קהלים גדולים מדי מסומנים (עם בלוקים מוצעים) במקום להיכשל בשקט.',
          '<strong style="color:#e9edef">הסכמה והסרה</strong> — אנשי קשר שביקשו הסרה או סומנו כספאם מוחרגים אוטומטית.',
          '<strong style="color:#e9edef">מודעות לחלון</strong> — שליחה המונית רגילה (טקסט חופשי) מגיעה רק לנמענים עם חלון פתוח; לתבניות מומלץ לקהלים קרים.',
          '<strong style="color:#e9edef">הגבלת קצב</strong> — כשמגיעים למגבלה, ה־API מחזיר מצב קריא למכונה כך שסוכנים נסוגים במקום להציף בניסיונות חוזרים.',
        ],
      },
      {
        title: 'עם סוכן AI (MCP)',
        prompts: [
          { user: 'שלח לכולם בגיליון הזה את תבנית shipping_update.' },
          { user: 'לכמה מ־5,000 המספרים האלה דיוור רגיל באמת יגיע?' },
          { user: 'שלח לכל מי שבפלח "נושרים" מחר.' },
        ],
        note: 'לקהלים גדולים, הסוכן מונחה אוטומטית לקמפיינים — הוא לא יבצע לולאת שליחות בודדות.',
      },
      {
        title: 'שלחו וואטסאפ בקנה מידה',
        text: 'צרו חשבון חינם, חברו וואטסאפ, והריצו קמפיין המוני תואם דרך API או MCP.',
        primary: { label: 'צור חשבון חינם →' },
        secondary: { label: 'הגדרת WhatsApp MCP' },
      },
    ],
    faq: [
      { q: 'מהי הדרך הנכונה לשלוח וואטסאפ בהמוניות?', a: 'השתמשו בקמפיינים (POST /campaigns ואז run, או POST /campaigns/send) — לא בלולאה על נקודות שליחה בודדות. קמפיינים מטפלים בתפוקה, במשתני תבנית לכל נמען, בהסכמה ובדיווח.' },
      { q: 'האם מגבלות המסרים של Meta נשמרות?', a: 'כן. תגובות הקמפיין חושפות את מדרגת המסרים היומית שלכם ומסמנות קהלים גדולים מדי (עם בלוקים יומיים מוצעים) במקום להיכשל בשקט.' },
      { q: 'האם אנשי קשר שביקשו הסרה מוחרגים?', a: 'כן — אנשי קשר שביקשו הסרה או סומנו כספאם מוחרגים אוטומטית, ונמענים יכולים להסיר את עצמם בתגובה.' },
      { q: 'מה קורה אם אני מגיע למגבלת קצב?', a: 'ה־API מחזיר מצב RATE_LIMITED / מגבלת־מסרים קריא למכונה כך שלקוחות וסוכני AI נסוגים ומנסים שוב מאוחר יותר במקום ללולאה.' },
    ],
  },
};
